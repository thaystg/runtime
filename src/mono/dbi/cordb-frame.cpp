...
CordbJITILFrame::GetReturnValueForILOffset (ULONG32 ILoffset, ICorDebugValue **ppReturnValue) {
	DEBUG_PRINTF (1, "CordbFrame - GetReturnValueForILOffset - NOT IMPLEMENTED\n");
	return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbJITILFrame::EnumerateLocalVariablesEx (ILCodeKind flags,
                                                                      ICorDebugValueEnum **ppValueEnum) {
	DEBUG_PRINTF (1, "CordbFrame - EnumerateLocalVariablesEx - NOT IMPLEMENTED\n");
	return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbJITILFrame::GetLocalVariableEx (ILCodeKind flags, DWORD dwIndex,
...
HRESULT STDMETHODCALLTYPE CordbJITILFrame::GetLocalVariable (
	...
	int cmdId = thread->ppProcess->connection->send_event (CMD_SET_STACK_FRAME, CMD_STACK_FRAME_GET_VALUES,
	                                                       &localbuf);
	buffer_free (&localbuf);
...
HRESULT STDMETHODCALLTYPE CordbJITILFrame::GetArgument (
	...
	buffer_add_int (&localbuf, dwIndex);
	int cmdId = thread->ppProcess->connection->send_event (CMD_SET_STACK_FRAME, CMD_STACK_FRAME_GET_ARGUMENT,
	                                                       &localbuf);
	buffer_free (&localbuf);
...
HRESULT STDMETHODCALLTYPE CordbNativeFrame::GetLocalRegisterValue (CorDebugRegister reg, ULONG cbSigBlob,
...
HRESULT STDMETHODCALLTYPE CordbNativeFrame::GetLocalDoubleRegisterValue (
	CorDebugRegister highWordReg, CorDebugRegister lowWordReg, ULONG cbSigBlob, PCCOR_SIGNATURE pvSigBlob,
	ICorDebugValue **ppValue) {
	DEBUG_PRINTF (1, "CordbNativeFrame - GetLocalDoubleRegisterValue - NOT IMPLEMENTED\n");
	return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbNativeFrame::GetLocalMemoryValue (CORDB_ADDRESS address, ULONG cbSigBlob,
...
HRESULT STDMETHODCALLTYPE CordbNativeFrame::GetLocalRegisterMemoryValue (
	CorDebugRegister highWordReg, CORDB_ADDRESS lowWordAddress, ULONG cbSigBlob, PCCOR_SIGNATURE pvSigBlob,
	ICorDebugValue **ppValue) {
...
HRESULT STDMETHODCALLTYPE CordbNativeFrame::GetLocalMemoryRegisterValue (
	CORDB_ADDRESS highWordAddress, CorDebugRegister lowWordRegister, ULONG cbSigBlob, PCCOR_SIGNATURE pvSigBlob,
	ICorDebugValue **ppValue) {
