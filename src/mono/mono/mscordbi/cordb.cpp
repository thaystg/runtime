...
MONO_API HRESULT CoreCLRCreateCordbObjectEx (int iDebuggerVersion, DWORD pid, LPCWSTR lpApplicationGroupId,
                                             HMODULE hmodTargetCLR, void **ppCordb) {
...
static void debugger_thread (void *ppProcess) {
	Connection *connection = new Connection ((CordbProcess*)ppProcess, ((CordbProcess*)ppProcess)->cordb);
	((CordbProcess*)ppProcess)->connection = connection;
	...
	for (int i = 0; i < nevents; ++i) {
		...
		case EVENT_KIND_ASSEMBLY_LOAD: {
			...
			if (pCorDebugAppDomain == NULL) {
				...
				ppCordb->pCallback->CreateAppDomain (static_cast<ICorDebugProcess*> (ppProcess),
				                                     pCorDebugAppDomain);
			}
		...
		case EVENT_KIND_BREAKPOINT: {
			...
			while (i < ppCordb->breakpoints->len) {
				...
				if (breakpoint->offset == offset && breakpoint->code->func->id == method_id) {
					ppCordb->pCallback->Breakpoint (pCorDebugAppDomain, thread,
					                                static_cast<ICorDebugFunctionBreakpoint*> (
						                                breakpoint));
					break;
				}
				i++;
		...
		case EVENT_KIND_STEP: {
			...
			if (!thread->stepper->isComplete)
				ppCordb->pCallback->StepComplete (pCorDebugAppDomain, thread, thread->stepper,
				                                  STEP_NORMAL);
