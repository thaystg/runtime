...
HRESULT STDMETHODCALLTYPE CordbEval::CallParameterizedFunction(ICorDebugFunction* pFunction, ULONG32 nTypeArgs, ICorDebugType* ppTypeArgs[], ULONG32 nArgs, ICorDebugValue* ppArgs[])
{
...
void CordbEval::EvalComplete(Buffer* localbuf2) {
	...
	conn->ppCordb->pCallback->EvalComplete(static_cast<ICorDebugAppDomain*>(g_ptr_array_index(thread->ppProcess->appdomains, 0)), static_cast<ICorDebugThread*>(thread), static_cast<ICorDebugEval*>(this));
}
...
HRESULT STDMETHODCALLTYPE CordbEval::NewParameterizedObject(ICorDebugFunction* pConstructor, ULONG32 nTypeArgs, ICorDebugType* ppTypeArgs[], ULONG32 nArgs, ICorDebugValue* ppArgs[])
{
	DEBUG_PRINTF(1, "CordbEval - NewParameterizedObject - NOT IMPLEMENTED\n");
	return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbEval::NewParameterizedObjectNoConstructor(ICorDebugClass* pClass, ULONG32 nTypeArgs, ICorDebugType* ppTypeArgs[])
{
	DEBUG_PRINTF(1, "CordbEval - NewParameterizedObjectNoConstructor - NOT IMPLEMENTED\n");
	return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbEval::NewParameterizedArray(ICorDebugType* pElementType, ULONG32 rank, ULONG32 dims[], ULONG32 lowBounds[])
{
...
ULONG CordbEval::AddRef(void) {
	return 1;
}

ULONG CordbEval::Release(void) {
	return 1;
}

HRESULT CordbEval::QueryInterface(REFIID id, _COM_Outptr_ void __RPC_FAR* __RPC_FAR* pInterface)
{
...
HRESULT STDMETHODCALLTYPE CordbEval::NewArray(CorElementType elementType, ICorDebugClass* pElementClass, ULONG32 rank, ULONG32 dims[], ULONG32 lowBounds[])
{
...
HRESULT STDMETHODCALLTYPE CordbEval::CreateValue(CorElementType elementType, ICorDebugClass* pElementClass, ICorDebugValue** ppValue)
{
	CordbContent content_value;
	content_value.booleanValue = 0;
	CordbValue* value = new CordbValue(conn, elementType, content_value, convert_mono_type_2_icordbg_size(elementType));
