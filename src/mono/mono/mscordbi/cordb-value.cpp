...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetString (ULONG32 cchString, ULONG32 *pcchString, WCHAR szString[]) {
	...
	if (type == ELEMENT_TYPE_STRING) {
		...
		*pcchString = cchString;
		int use_utf16 = decode_byte (localbuf2->buf, &localbuf2->buf, localbuf2->end);
		if (use_utf16) {
			DEBUG_PRINTF (1, "CordbObjectValue - GetString - NOT IMPLEMENTED - use_utf16\n");
		}
		else {
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetCachedInterfacePointers (BOOL bIInspectableOnly, ULONG32 celt,
                                                                        ULONG32 *pcEltFetched, CORDB_ADDRESS *ptrs) {
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetFieldValue (ICorDebugClass *pClass, mdFieldDef fieldDef,
	...
	case VALUE_TYPE_ID_NULL: {
		...
		if (type == MONO_TYPE_SZARRAY) {
			...
			if (type_id == ELEMENT_TYPE_CLASS) {
				...
				char *namespace_str = decode_string (localbuf2->buf, &localbuf2->buf, localbuf2->end);
				char *class_name_str = decode_string (localbuf2->buf, &localbuf2->buf, localbuf2->end);
				char *class_fullname_str = decode_string (
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetFieldValue (ICorDebugClass *pClass, mdFieldDef fieldDef,
                                                          ICorDebugValue **ppValue) {
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetCachedInterfacePointers (BOOL bIInspectableOnly, ULONG32 celt,
                                                                       ULONG32 *pcEltFetched, CORDB_ADDRESS *ptrs) {
